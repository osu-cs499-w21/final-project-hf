...
export const media = Object.keys(sizes).reduce((acc: Record<string, MediaFn>, label) => {
  acc[label] = (arg1: TemplateStringsArray | CSSObject, ...args: SimpleInterpolation[]) => css`
...
export const gridCss = (gridSize: number, useDisplayGrid = false): string =>
  useDisplayGrid ? `grid-column: span ${gridSize};` : `width: ${grid(gridSize)};`;

const gridSizesToCss = (gridSizes: Record<string, number>, useDisplayGrid: boolean) =>
  Object.entries(gridSizes)
    .sort((a, b) => sizes[a[0]] - sizes[b[0]])
    .map(([mediaSize, gridSize]) => media[mediaSize]`${gridCss(gridSize, useDisplayGrid)}`);
